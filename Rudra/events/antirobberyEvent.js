...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  // Ensure AntiRobbery is enabled for this group
  ...
  // Check if admin changes exist in logMessageData
  const { addedParticipants, removedParticipants } = logMessageData;
  ...
  if (addedParticipants && addedParticipants.length > 0) {
    ...
    for (let addedUser of addedParticipants) {
      ...
      if (addedUser.userFbId !== global.config.ADMINBOT[0]) {
        api.sendMessage(
          `» [ Admin Update ]: Admin added - ${addedUser.userFbId}`,
          threadID
        );
      }
    }
